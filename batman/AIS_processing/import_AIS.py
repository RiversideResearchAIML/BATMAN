...
def extract_df( source = None, format = 'feather', update = False, overwrite = False, delete_source = False, nrows = None, pbar = None ):
    out_file = os.path.join( AIS_parameters.dirs['root'], 'ingested', os.path.splitext(os.path.split(source)[1])[0]+' (%s).'%fname[:-3]+format)
...
def get_df( date = '2022_06_29', format = 'feather', pbar = None, multicolumn = False, nrows = None, columns = None ):
    '''
    ...
    if isinstance( date, pd.core.indexes.datetimes.DatetimeIndex ):
        date = list( pd.date_range(date[0], date[-1], freq='D') )
    if isinstance(date, list):
        ...
        for date_ in pbar_:
            ...
            df_dict[date_], filename_dict[date_] = get_df( date_, format, pbar, nrows = nrows, columns = columns )
        ...
        df, out_file = extract_df( source = source_file, format = format, pbar = pbar )
        if not nrows is None:
            df = df.iloc[:int(nrows)]
        if not columns is None:
            if isinstance(columns, str) and (columns.lower() == 'numeric'):
                columns = ['timestamp'] + [col for col, dtype in df.dtypes.items() if (pd.api.types.is_numeric_dtype(dtype))]
            elif isinstance(columns, str) and (columns.lower().__contains__('non')):
                columns = [col for col, dtype in df.dtypes.items() if not (pd.api.types.is_numeric_dtype(dtype))]
            df = df[columns]
        ##
